...
(function () {
  ...
  document.body.innerHTML = [
    ...
    '    <button class="reload-btn" id="reload-btn" aria-label="再読み込み"><svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M23 4v6h-6"/><path d="M20.49 15a9 9 0 1 1-2.12-9.36L23 10"/></svg></button>',
    '  </div>',
    ...
    '</header>',
    '<main>',
    '  <div class="grid" id="grid"><div class="state"><span class="ico">💕</span>読み込み中…</div></div>',
  ...
  function thumbUrl(url) { return url.replace(/\/upload\/[^/]*\//, "/upload/c_fill,g_auto,w_400,h_520,q_auto,f_auto/"); }
  function fullUrl(url) { return url.replace(/\/upload\/[^/]*\//, "/upload/c_limit,w_1100,h_1800,q_auto,f_auto/"); }
  ...
  function switchNav(filter, label, btnEl) {
    ...
    activeFilter = filter; $("header-title").textContent = label;
    $("search").value = ""; searchQ = ""; currentPage = 1;
    render(); window.scrollTo({ top: 0, behavior: "smooth" });
  }

  // ---- データ取得（GAS。localStorageでstale-while-revalidate） ----
  function buildCharaList(data) {
    var seen = {}, list = [];
    data.forEach(function(d){ if (d.chara && !seen[d.chara]) { seen[d.chara] = true; list.push(d.chara); } });
    return list;
  }
  ...
  function fetchData(forceReload) {
    ...
    fetch(CONFIG.gasUrl + "?folder=" + encodeURIComponent(CONFIG.folder))
      ...
      .then(function(res){
        if (res.status !== "ok") {
          if (!hasCache) showState("⚠️", res.message || "読み込み失敗");
          return;
        }
        var fresh = res.rows || [];
        var freshStr = JSON.stringify({ data: fresh });
        if (freshStr !== JSON.stringify({ data: allData })) applyData(fresh);
        try { localStorage.setItem(CACHE_KEY, freshStr); } catch (e) {}
      })
      .catch(function(e){
        if (!hasCache) showState("⚠️", "読み込みに失敗しました：" + e.message);
      });
  }
  ...
  function getFiltered() {
    ...
    return allData.filter(function(d){
      ...
      if (!q) return true;
      return matchSearch(d.code, q) || matchSearch(d.chara, q);
    });
  }
  ...
  function render() {
    ...
    page.forEach(function(d){
      ...
      card.className = "card";
      var imgHtml = d.img
        ? '<img src="' + esc(thumbUrl(d.img)) + '" alt="' + esc(d.code) + '" loading="lazy" decoding="async" onload="this.classList.add(\'loaded\')">'
        : '<div class="no-img">' + HEART_ICO + '</div>';
      card.innerHTML = '<div class="card-img">' + imgHtml + '</div>'
        + '<div class="card-body"><div class="card-chara">' + esc(d.chara) + '</div>'
        + '<div class="card-name">' + esc(d.code) + '</div></div>';
      card.onclick = function(){ openModal(d); };
      frag.appendChild(card);
    });
    var g = $("grid"); g.innerHTML = ""; g.appendChild(frag);
    renderPag(total);

    if (currentPage < total) {
      var next = list.slice(currentPage * PAGE_SIZE, (currentPage + 1) * PAGE_SIZE);
      next.forEach(function(d){ if (d.img) { var im = new Image(); im.src = thumbUrl(d.img); } });
    }
  }
  ...
  function openModal(d) {
    var iw = $("m-img");
    iw.innerHTML = d.img
      ? '<img src="' + esc(fullUrl(d.img)) + '" alt="' + esc(d.code) + '">'
